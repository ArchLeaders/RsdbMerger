...
/// <summary>
/// Common <see cref="RsdbUniqueRowMerger"/> instances.
/// </summary>
public abstract class RsdbUniqueRowMergers
{
    private const string ROW_ID_KEY = "__RowId";
    public static readonly RsdbUniqueRowMerger RowId = new(ROW_ID_KEY,
        (row) => XxHash3.HashToUInt64(MemoryMarshal.Cast<char, byte>(row[ROW_ID_KEY].GetString())));

    private const string NAME_KEY = "Name";
    public static readonly RsdbUniqueRowMerger Name = new(NAME_KEY,
        (row) => XxHash3.HashToUInt64(MemoryMarshal.Cast<char, byte>(row[NAME_KEY].GetString())));
    ...
    public static readonly RsdbUniqueRowMerger FullTagId = new(FULL_TAG_ID_KEY,
        (row) => XxHash3.HashToUInt64(MemoryMarshal.Cast<char, byte>(row[FULL_TAG_ID_KEY].GetString())),
    ...
    public static readonly RsdbUniqueRowMerger NameHash = new(NAME_HASH_KEY,
        (row) => row[NAME_HASH_KEY].GetUInt32());
}
